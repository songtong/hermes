...
import com.ctrip.hermes.admin.core.converter.EntityToViewConverter;
...
public class TopicService {
	...
	public TopicView addPartitionsForTopic(String topicName, List<Partition> partitions) throws Exception {
		...
		com.ctrip.hermes.admin.core.model.Topic topicModel = m_topicDao.findByName(topicName);

		for (Partition partition : partitions) {
			com.ctrip.hermes.admin.core.model.Partition partitionModel = EntityToModelConverter.convert(partition);
			if (partition.getId() < 0) {
				...
				if (Storage.MYSQL.equals(topic.getStorageType())) {
					Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup> consumerGroupModels = m_consumerGroupDao
	...
	public TopicView createTopic(TopicView topicView) throws Exception {
		...
		com.ctrip.hermes.admin.core.model.Topic topicModel = ViewToModelConverter.convert(topicView);
		Collection<com.ctrip.hermes.admin.core.model.Partition> partitionModels = new ArrayList<>();
		try {
			...
			for (com.ctrip.hermes.meta.entity.Partition partitionEntity : topicView.getPartitions()) {
				com.ctrip.hermes.admin.core.model.Partition partitionModel = EntityToModelConverter
	...
	public void deleteTopic(String name) throws Exception {
		...
		if (topic == null)
			return;
		Collection<com.ctrip.hermes.admin.core.model.Partition> partitions = new ArrayList<>();
		for (com.ctrip.hermes.meta.entity.Partition partitionEntity : topic.getPartitions()) {
			com.ctrip.hermes.admin.core.model.Partition partitionModel = EntityToModelConverter.convert(partitionEntity);
			partitionModel.setTopicId(topic.getId());
			partitionModel.setTId(topic.getId());
			m_partitionDao.deleteByTopicId(partitionModel);
			partitions.add(partitionModel);
		}
		Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup> consumerGroups = new ArrayList<>();
		for (com.ctrip.hermes.meta.entity.ConsumerGroup cgEntity : topic.getConsumerGroups()) {
			com.ctrip.hermes.admin.core.model.ConsumerGroup cgModel = EntityToModelConverter.convert(cgEntity);
		...
		for (com.ctrip.hermes.meta.entity.Producer producerEntity : topic.getProducers()) {
			com.ctrip.hermes.admin.core.model.Producer producerModel = EntityToModelConverter.convert(producerEntity);
			m_producerDao.deleteByPK(producerModel);
		}

		com.ctrip.hermes.admin.core.model.Topic topicModel = EntityToModelConverter.convert(topic);
	...
	public Topic findTopicEntityById(long id) {
		try {
			com.ctrip.hermes.admin.core.model.Topic model = this.m_topicDao.findByPK(id);
	...
	public Topic findTopicEntityByName(String topic) {
		try {
			com.ctrip.hermes.admin.core.model.Topic model = this.m_topicDao.findByName(topic);
	...
	public TopicView findTopicViewByName(String topicName) {
		try {
			com.ctrip.hermes.admin.core.model.Topic topicModel = this.m_topicDao.findByName(topicName);
	...
	public List<String> getTopicNames() {
		...
		try {
			Collection<com.ctrip.hermes.admin.core.model.Topic> topicModels = m_topicDao.list(false);
			for (com.ctrip.hermes.admin.core.model.Topic topicModel : topicModels) {
	...
	public TopicView updateTopic(TopicView topicView) throws Exception {
		com.ctrip.hermes.admin.core.model.Topic topicModel = this.m_topicDao.findByName(topicView.getName());
		...
		topicModel.setIdcPolicy(topicView.getIdcPolicy());
	...
	public List<com.ctrip.hermes.meta.entity.Topic> findTopicEntities(boolean isFillDetail) throws DalException {
		Collection<com.ctrip.hermes.admin.core.model.Topic> models = m_topicDao.list(true);
		Map<Long, Collection<ConsumerGroup>> consumers = consumerListToMap(m_consumerGroupDao.list(true));
		Map<Long, Collection<com.ctrip.hermes.admin.core.model.Partition>> partitions = partitionListToMap(m_partitionDao
		      .list(true));
	...
	private void addConsumerGroups4Topic(Topic topic, Collection<ConsumerGroup> consumers) {
		if (consumers != null) {
			for (com.ctrip.hermes.admin.core.model.ConsumerGroup model : consumers) {
	...
	private void addPartitions4Topic(Topic topic, Collection<com.ctrip.hermes.admin.core.model.Partition> partitions) {
		if (partitions != null) {
			for (com.ctrip.hermes.admin.core.model.Partition model : partitions) {
	...
	private void addPartitions4TopicView(TopicView topicView,
	      Collection<com.ctrip.hermes.admin.core.model.Partition> partitions) {
		if (partitions != null) {
			for (com.ctrip.hermes.admin.core.model.Partition model : partitions) {
				com.ctrip.hermes.meta.entity.Partition entity = ModelToEntityConverter.convert(model);
	...
	private Map<Long, Collection<com.ctrip.hermes.admin.core.model.Partition>> partitionListToMap(
	      Collection<com.ctrip.hermes.admin.core.model.Partition> collection) {
		Map<Long, Collection<com.ctrip.hermes.admin.core.model.Partition>> map = new HashMap<>();
		if (collection != null) {
			for (com.ctrip.hermes.admin.core.model.Partition partition : collection) {
				Collection<com.ctrip.hermes.admin.core.model.Partition> list = map.get(partition.getTopicId());
				if (list == null) {
					list = new ArrayList<>();
	...
	private Map<Long, Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup>> consumerListToMap(
	      Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup> collection) {
		Map<Long, Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup>> map = new HashMap<>();
		if (collection != null) {
			for (com.ctrip.hermes.admin.core.model.ConsumerGroup consumer : collection) {
				Collection<com.ctrip.hermes.admin.core.model.ConsumerGroup> list = map.get(consumer.getTopicId());
				if (list == null) {
					list = new ArrayList<>();
