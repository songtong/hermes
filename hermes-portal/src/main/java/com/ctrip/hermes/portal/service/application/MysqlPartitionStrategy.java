...
public class MysqlPartitionStrategy extends PartitionStrategy {
	...
	protected StrategyDatasource getDefaultDatasource(TopicApplication application) throws DalException {
		...
		if (datasourcesTags == null || datasourcesTags.size() == 0) {
			if (tag != defaultTag) {
				...
				if (datasourcesTags == null || datasourcesTags.size() == 0) {
					throw new RuntimeException(String.format("Default tag on group %s can NOT find any mapping datasource!",
					      DEFAULT_TAG_GROUP));
				}
			}
		}
		...
		if (datasourcesTags.size() == 1) {
			return StrategyDatasource.newInstance(
			      Pair.from(datasourcesTags.get(0).getDatasourceId(), datasourcesTags.get(0).getDatasourceId()), tags);
		}
	...
	protected String getDefaultBrokerGroup(TopicApplication application) {
		...
		String defaultBrokerGroup = "default";
		if (brokerGroups.contains(application.getProductLine().toLowerCase())) {
			defaultBrokerGroup = application.getProductLine().toLowerCase();
		}
