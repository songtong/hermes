application_module.controller('app-approval-detail-controller', [ '$scope', '$routeParams', '$resource', 'ApplicationService', '$location', '$window', '$q', '$filter', '$resource', 'watcher', 'TopicSync', 'clone', 'cache', 'promiseChain', 'user',
		function($scope, $routeParams, $resource, ApplicationService, $location, $window, $q, $filter, $resource, watcher, TopicSync, clone, cache, promiseChain, user) {
			// Default options.
			$scope.env = 'fws';
			...
			$scope.createTopic = function() {
				...
				// Set timeout to 10 seconds, and set maximum to 80 if it's a
				// mysql topic, otherwise 30.
				$scope.$broadcast('progress-random', 'syncProgressBar', $scope.view.storageType == 'kafka' ? 3000 : 10000, $scope.view.storageType == 'kafka' ? 30 : 80);
				...
				topicChain.add({
					...
					success : function(result) {
						...
						decodeCodec($scope.views[env]);
						$scope.view = $scope.views[env];

						if ($scope.view.storageType == 'mysql') {
							$scope.$broadcast('progress', 'syncProgressBar', {
								percentage : 90,
								msg : 'Topic created on env: ' + $scope.env
							});
						} else {
							$scope.$broadcast('progress', 'syncProgressBar', {
								percentage : 50,
				...
				if ($scope.view.storageType == 'kafka') {
					topicChain.add({
						...
						success : function(result) {
							$scope.$broadcast('progress', 'syncProgressBar', {
								percentage : 90,
								msg : 'Topic deployed on env: ' + $scope.env
							});
							finishWatcher.step();
						},
						error : handleError
			...
			$scope.passApplication = function passApplication() {
				...
				ApplicationService.pass_application($scope.application.id, encodeComment(), user.sn, $scope.application).then(function(result) {
					$scope.application = result;
					decodeComment();
					// When the application is passed, invoke api to get dynamic
					// view.
					$scope.openSyncModal();
				}, function(result) {
				});
			...
			$scope.openSyncModal = function() {
				...
				function checkOnEnv(env) {
					...
					if ($scope.application["type"] == 0) {
						...
						// For creation requests, call storage api to get list
						// serving config modal view.
						remoteChain.add({
							func : TopicSync.getStorage,
							args : [ env, {
								type : $scope.application.storageType
							} ],
							...
							success : function(result) {
								...
								// If this is the view of wanted env, set
								// current view to default one.
								if (env == $scope.env) {
									$scope.view = $scope.views[env];
								}

								// Trigger watcher to notify the end of one
								// chain invocation.
								finishWatcher.step();
							...
							success : function(result) {
								...
								console.log($scope.tags);
								// Keep one selection position for each tag
								// group.
								// $scope.selectedTags = new
								// Array(Object.keys($scope.tags).length);
							}
						}).add({
						...
						remoteChain.add({
							...
							success : function(result) {
								// Check whether specified consumer is already
								// existed.
								var consumers = result.filter(function(elem) {
									return elem['name'] == consumerName;
								});

								// If it's already created, abort the call
								// chain.
