application_module.controller('app-review-list-controller', [ '$scope', 'ApplicationService', function($scope, ApplicationService) {
	$scope.applications = [];
	$scope.status = '0';
	...
	$scope.paginate = function(state) {
		...
		ApplicationService.get_applications($scope.status, ssoMail, start, size).then(function(result) {
