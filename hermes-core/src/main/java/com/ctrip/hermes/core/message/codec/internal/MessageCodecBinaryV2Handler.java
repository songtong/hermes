 ...
 *
 */
public class MessageCodecBinaryV2Handler implements MessageCodecHandler {
	...
	private void encode(ProducerMessage<?> msg, ByteBuf buf, byte[] body, String codecType) {
		...
		codec.writeBoolean(msg.isWithHeader());
		if (msg.isWithHeader()) {
			// header begin
	...
	public PartialDecodedMessage decodePartial(ByteBuf buf) {
		...
		msg.setWithHeader(codec.readBoolean());
	...
	public BaseConsumerMessage<?> decode(String topic, ByteBuf buf, Class<?> bodyClazz) {
		...
		msg.setTopic(topic);
		if (decodedMessage.isWithHeader()) {
