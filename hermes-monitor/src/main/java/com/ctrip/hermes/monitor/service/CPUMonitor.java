...
public class CPUMonitor {
	...
	private void monitorCPU(Date timeFrom, Date timeTill, String group) throws ZabbixApiException {
	...
	public void monitorHourly() throws ZabbixApiException, DalException {
		...
		for (String group : groups) {
			monitorCPU(timeFrom, timeTill, group);
		}
	}
	...
	private Map<Integer, StatResult> statCPUUserTime(Date timeFrom, Date timeTill, Map<Integer, HostObject> hosts)
		...
		Map<Integer, List<ItemObject>> ids = ZabbixApiUtils.searchItemsByKey(hosts.keySet(), ZabbixConst.CPU_USER_TIME);
		...
		for (Integer hostid : hosts.keySet()) {
			Map<Integer, StatResult> statResults = ZabbixStatUtils.getHistoryStat(timeFrom, timeTill, hostid,
			      ids.get(hostid), HISOTRY_OBJECT_TYPE.FLOAT);
	...
	private Map<Integer, StatResult> statCPUSystemTime(Date timeFrom, Date timeTill, Map<Integer, HostObject> hosts)
	      throws ZabbixApiException {
		Map<Integer, List<ItemObject>> ids = ZabbixApiUtils.searchItemsByKey(hosts.keySet(), ZabbixConst.CPU_SYSTEM_TIME);
		Map<Integer, StatResult> result = new HashMap<Integer, StatResult>();
		for (Integer hostid : hosts.keySet()) {
			Map<Integer, StatResult> statResults = ZabbixStatUtils.getHistoryStat(timeFrom, timeTill, hostid,
			      ids.get(hostid), HISOTRY_OBJECT_TYPE.FLOAT);
	...
	private Map<Integer, StatResult> statCPUIOWaitTime(Date timeFrom, Date timeTill, Map<Integer, HostObject> hosts)
	      throws ZabbixApiException {
		Map<Integer, List<ItemObject>> ids = ZabbixApiUtils.searchItemsByKey(hosts.keySet(), ZabbixConst.CPU_IOWAIT_TIME);
		Map<Integer, StatResult> result = new HashMap<Integer, StatResult>();
		for (Integer hostid : hosts.keySet()) {
			Map<Integer, StatResult> statResults = ZabbixStatUtils.getHistoryStat(timeFrom, timeTill, hostid,
			      ids.get(hostid), HISOTRY_OBJECT_TYPE.FLOAT);
	...
	private Map<Integer, StatResult> statCPURatioLoadOfNumber(Date timeFrom, Date timeTill,
		...
		Map<Integer, List<ItemObject>> ids = ZabbixApiUtils.searchItemsByName(hosts.keySet(),
