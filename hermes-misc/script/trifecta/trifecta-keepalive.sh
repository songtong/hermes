...
function restart_trifecta {
        ...
        nohup java -Xmx1G -jar trifecta_0.18.20.bin.jar --http-start >/dev/null 2>/opt/logs/trifecta/nohup.log &
}
...
curl --connect-timeout 5 --max-time 3 http://127.0.0.1:8888 >/dev/null 2>&1
if [ $? != 0 ];then
